...
(documented in each row, and in that run's own `NOTE.md` where the scripting needs explaining --
one of the five, the schema-validation row, never touches a browser or the mock app at all, so it
has no NOTE.md). In every one of those five, only the *surface being observed* (and, for the two
discovery-handoff rows, the model) is scripted; the engine, schema, and broker are the real code,
unmodified.
...
| `replay-20260917214233/` | `cua replay --artifact .../2.json -p member_id=12345 --fault "inject=500"`, then `cua ops claim`, `cua ops release --note "reloaded the page and it looks fixed now -- should be good to resume"` -- a good-faith note, but the browser was never actually touched between claim and release | `FAILED` on resume (the fault is still there). `human_action.json`'s `before`/`after` URLs are identical (`?inject=500` both times) and `resume_decision: "none"` / `human_performed_pending_action: false` -- mechanically derived from re-checking the real page, disagreeing with the operator's own good-faith note that it looked fixed. |
...
| `recovery-refused-schema-20260915234907/` | attempted `Capability.model_validate(...)` on a hand-built artifact declaring `recovery.do="retry_step"` on a Click step | Real `pydantic.ValidationError` (`validation_error.txt`) -- the schema-level half of the retry_step guard (REPORT.md sec 3) refuses to let such an artifact be constructed at all, let alone saved. **Scripted, no browser** -- no `NOTE.md` here: this row never touches a browser or the mock app in the first place, so there's no real-vs-scripted split to explain. |
...
| `session-lost-20260915234907/` | real `replay()` + a real SQLite `ControlBroker` (two independent connections, one per thread, mirroring the two real OS processes `cua replay`/`cua ops` actually are) -- claim, then release with a note but no real fix, on a surface that reports an off-scope location after the "crash" | `FAILED`, kind `session_lost`, `observed: "now at http://evil.example.com/login"`. Also produced real `intervention.json` and `human_action.json` -- the same `resume_decision`/`human_performed_pending_action` machinery as `replay-20260917214233/` runs here too, this time against a scope check rather than a checkpoint. **Scripted surface** -- see `NOTE.md`. |
...
This is the eighth generation of this evidence set. Four rounds of review found
...
against the count this file claimed. An eighth round replaced `replay-20260915230202/`
with `replay-20260917214233/`: the same close-reading pass found that the old run's own
`human_action.json` didn't actually demonstrate what this file and REPORT.md sec 5 both
claimed it did (an honest operator note that already said "fault still present" was being
cited as disagreeing with the mechanically-derived fact, when it agreed with it). Rather
than soften the claim, the run was redone with a genuinely good-faith-but-wrong note --
real `cua replay`/`cua ops claim`/`cua ops release`, fault left untouched -- so the evidence
now actually backs the point being made about it.
...
- `human_action.json` (handoff only) -- what the operator actually did: who claimed
  ...
  `replay-20260917214233/` exists specifically to show the derived fact disagreeing
...
- `NOTE.md` -- present in four of the five scripted rows (`recovery-refused-runtime`,
  `session-lost`, both `discovery-handoff-*`; the fifth, `recovery-refused-schema`, needs
  none -- see its own row above), explaining what's real and what's scripted in that run
  and why the mock target app can't produce that condition live. A claim of "real" needs a
  place to be honest about the one part that isn't. `replay-20260915235033/` also has one,
  for an unrelated reason -- that row is fully live, not scripted; its `NOTE.md` explains
  why resetting an already-approved artifact to `draft` to produce this evidence is a
  legitimate, not contrived, way to get there.
